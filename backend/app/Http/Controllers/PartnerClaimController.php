...
class PartnerClaimController extends Controller
{
     ...
     *
     * Accepts the obvious spellings of both, because the alternative is a round
     * trip with their engineers over whether it is `uid` or `user_id` — and the
     * link is generated by their code, not ours.
     *
     ...
     *   - Referrer-Policy: no-referrer on the page, so it does not leak
     *     sideways to anything the page loads;
    ...
    {
        $id   = $this->firstOf($request, ['uid', 'user_id', 'userid', 'external_user_id', 'id']);
        $code = $this->firstOf($request, ['code', 'activation_code', 'activationcode', 'ac']);
     ...
     *
     * Belt and braces next to the redirect: these pages carry a credential in a
     * form field, and a stray third-party script or image is the classic way
     * that ends up somewhere it should not be.
     */
