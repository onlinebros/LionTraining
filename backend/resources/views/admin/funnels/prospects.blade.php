...
@section('title', $funnel->title.' — who went through')
@section('page-title', $funnel->title)

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('admin.funnels.index') }}">Funnels</a></li>
    <li class="breadcrumb-item"><a href="{{ route('admin.funnels.show', $funnel) }}">{{ $funnel->title }}</a></li>
    <li class="breadcrumb-item active">Who went through</li>
@endsection
...
<div class="row g-3">
    <div class="col-lg-7">
        <div class="card">
            <div class="card-header">
                ...
                <small class="text-muted">Where each person got to, and the path they took.</small>
            </div>
            <div class="card-body">
                        ...
                        @forelse($people as $p)
                            <tr>
                                <td>
                                    ...
                                    @if($p['host'])
                                        <div class="text-muted small">via {{ $p['host'] }}</div>
                                    @endif
                                </td>
                                <td>
                                    <div>{{ $p['step'] ?? '—' }}</div>
                                    ...
                                    @if($p['watching'])
                                        <span class="tag tag--live mt-1">Watching now</span>
                                    @endif
                                </td>
                                <td>
                                    @if($p['path'])
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="text-center text-muted py-5">
                                    Nobody has been through this flow yet.
                                </td>
                            </tr>
                        @endforelse
                        </tbody>
    ...
    <div class="col-lg-5">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-1">What people choose</h5>
                <small class="text-muted">
                    A branch nobody takes is a video not worth making. One everybody takes is the
                    thing to say sooner.
                </small>
            </div>
            <div class="card-body">
