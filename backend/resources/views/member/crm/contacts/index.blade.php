...
@section('page-title', 'My Contacts')
    ...
    <div class="col-sm-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <a href="{{ route('member.crm.contacts.create') }}" class="btn btn-primary btn-sm">
                ...
                <form method="GET" class="row g-2 align-items-end">
                    ...
                    <div class="col-md-2 d-flex gap-1">
                        <button type="submit" class="btn btn-primary btn-sm">
                            <i data-feather="search" data-width="13" data-height="13"></i> Search
                        </button>
                        <a href="{{ route('member.crm.contacts.index') }}" class="btn btn-outline-secondary btn-sm">Clear</a>
                    </div>
                </form>
            </div>
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            @forelse($contacts as $c)
                                <tr>
                                    <td>
                                        <a href="{{ route('member.crm.contacts.show', $c) }}" class="fw-semibold text-dark">
                                            {{ $c->full_name }}
                                        </a>
                                        @if($c->email)<div class="small f-light">{{ $c->email }}</div>@endif
                                        @if($c->phone)<div class="small f-light">{{ $c->phone }}</div>@endif
                                    </td>
                                    <td><span class="badge badge-light-secondary small">{{ \App\Models\CrmContact::$contactTypes[$c->contact_type] ?? $c->contact_type }}</span></td>
                                    <td>
                                    ...
                                    <td>
                                        @foreach($c->tags->take(3) as $tag)
                                            <span class="badge me-1" style="background:{{ $tag->color }};color:#fff;font-size:.65rem;">{{ $tag->name }}</span>
                                        @endforeach
                                    </td>
                                    ...
                                    <td class="text-nowrap">
                                        <div class="d-flex gap-1 justify-content-end">
                                            ...
                                            <a href="{{ route('member.crm.contacts.edit', $c) }}" class="btn btn-warning btn-sm">
                                                <i data-feather="edit" data-width="13" data-height="13"></i>
                                            </a>
                                            <form method="POST" action="{{ route('member.crm.contacts.destroy', $c) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center f-light py-4">
                                        No contacts found.
                                        <a href="{{ route('member.crm.contacts.create') }}">Add your first contact →</a>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
